...
const analyzeContractWithAI = (contractText, contractType, userId, tier) => __awaiter(void 0, void 0, void 0, function* () {
  ...
  `;
    try {
        ...
        let processedAnalysis = Object.assign(Object.assign({}, analysis), { createdAt: new Date(analysis.createdAt || new Date().toISOString()), expirationDate: new Date(analysis.expirationDate || Date.now() + 30 * 24 * 60 * 60 * 1000), version: analysis.version || 1, userFeedback: analysis.userFeedback || { rating: 0, comments: "" }, customFields: analysis.customFields || {}, language: analysis.language || "en", aiModel: analysis.aiModel || AI_MODEL });
        // Calculate overallScore if missing
        if (processedAnalysis.overallScore === undefined ||
            Number.isNaN(processedAnalysis.overallScore)) {
            const riskScore = processedAnalysis.risks.reduce((sum, r) => {
                return (sum + (r.severity === "high" ? 20 : r.severity === "medium" ? 10 : 5));
            }, 0);
            const opportunityScore = processedAnalysis.opportunities.reduce((sum, o) => {
                return (sum + (o.impact === "high" ? 20 : o.impact === "medium" ? 10 : 5));
            }, 0);
            processedAnalysis.overallScore = Math.max(0, Math.min(100, 50 - riskScore + opportunityScore));
        }
        // Fill missing fields for premium tier
        if (tier === "premium") {
            // Infer contractDuration if missing
            if (!processedAnalysis.contractDuration) {
                const durationMatch = contractText.match(/(?:duration|term)\s*:\s*(\d+\s*(?:year|month|day)s?)/i);
            ...
            if (!processedAnalysis.terminationConditions) {
                const terminationMatch = contractText.match(/(?:termination|notice period)\s*:\s*(\d+\s*days)/i);
                processedAnalysis.terminationConditions = terminationMatch
                    ? terminationMatch[1]
                    : "Not specified";
            }
            // Infer legalCompliance if missing
            if (!processedAnalysis.legalCompliance ||
            ...
            if (!processedAnalysis.keyClauses ||
            ...
            if (!processedAnalysis.recommendations ||
                processedAnalysis.recommendations.length === 0) {
                processedAnalysis.recommendations = processedAnalysis.risks.map((r) => `Mitigate risk: ${r.risk}`);
                if (processedAnalysis.recommendations.length === 0) {
                    processedAnalysis.recommendations.push("No recommendations available");
                }
            }
            ...
            if (typeof processedAnalysis.intellectualPropertyClauses === "string") {
                ...
                processedAnalysis.intellectualPropertyClauses = contractText.includes("intellectual property")
                    ? ["IP ownership clause"]
                    : [];
            }
            // Ensure negotiationPoints is populated
            if (!processedAnalysis.negotiationPoints ||
            ...
            if (!processedAnalysis.performanceMetrics ||
        ...
        const fallbackAnalysis = {
            ...
            overallScore: 0,
            compensationStructure: tier === "premium"
                ? {
                    baseSalary: "Not specified",
                    bonuses: "Not specified",
                    equity: "Not specified",
            ...
            language: "en",
            aiModel: AI_MODEL,
            financialTerms: tier === "premium" ? { description: "", details: [] } : undefined,
        };
        let text = "";
        try {
            const results = yield aiModel.generateContent(prompt);
        ...
        const opportunitiesMatch = text.match(/"opportunities"\s*:\s*\[([\s\S]*?)\](?=\s*,|\s*})/);
        ...
        if (overallScoreMatch) {
            ...
            const opportunityScore = fallbackAnalysis.opportunities.reduce((sum, o) => {
                return (sum + (o.impact === "high" ? 20 : o.impact === "medium" ? 10 : 5));
            }, 0);
            fallbackAnalysis.overallScore = Math.max(0, Math.min(100, 50 - riskScore + opportunityScore));
        }
        if (tier === "premium") {
            const keyClausesMatch = text.match(/"keyClauses"\s*:\s*\[([\s\S]*?)\](?=\s*,|\s*})/);
            if (keyClausesMatch) {
                try {
            ...
            const recommendationsMatch = text.match(/"recommendations"\s*:\s*\[([\s\S]*?)\](?=\s*,|\s*})/);
            ...
            const legalComplianceMatch = text.match(/"legalCompliance"\s*:\s*\[([\s\S]*?)\](?=\s*,|\s*})/);
            ...
            const contractDurationMatch = text.match(/"contractDuration"\s*:\s*"([^"]*)"/);
            ...
            const terminationConditionsMatch = text.match(/"terminationConditions"\s*:\s*"([^"]*)"/);
